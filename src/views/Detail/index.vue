          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="saleAttr in spuSaleAttrList" :key="saleAttr.id">
                ...
                <dd changepirce="0"
                 :class="supsaleAttr.isChecked==='1'?'active':''"
                 v-for="supsaleAttr in saleAttr.spuSaleAttrValueList"
                 :key="supsaleAttr.id"
                 @click="chooseAttr(saleAttr,supsaleAttr.saleAttrValueName)"
                 >{{supsaleAttr.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
...
export default {
  ...
  created () {
    this.inintDetailList()
  },
  methods: {
    inintDetailList () {
      this.$store.dispatch('detail/getDetailList', this.$route.params.id)
    },
    chooseAttr (attr, value) {
      attr.spuSaleAttrValueList.some(item => {
        if (item.saleAttrValueName === value) {
          item.isChecked = '1'
        } else {
          item.isChecked = '0'
        }
    ...
    // 添加购物车
    async addCar () {
      // 1.告知服务器要添加到购物车保存数据，2.得到反馈结果 成功跳转路由 失败提示信息
      const ret = await this.$store.dispatch('detail/addOrUpdateCar', { skuId: this.$route.params.id, skuNum: this.skuNum })
      console.log(ret)
      // 成功跳转路由到购物车页面
      if (ret === 'ok') {
        // 利用本地会话存储读取产品详情
